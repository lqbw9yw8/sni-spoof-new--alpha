...
impl Fe {
    ...
    /// a - b + 2p (the bias is 0 mod p, so the result is unreduced but
    /// positive). Constants from curve25519-donna-c64's fsub.
    fn sub(&self, o: &Fe) -> Fe {
        Fe([
            self.0[0] + 0xFFFF_FFFF_FFFF_FFDA - o.0[0],
            self.0[1] + 0xFFFF_FFFF_FFFF_FFFE - o.0[1],
            self.0[2] + 0xFFFF_FFFF_FFFF_FFFE - o.0[2],
            self.0[3] + 0xFFFF_FFFF_FFFF_FFFE - o.0[3],
            self.0[4] + 0xFFFF_FFFF_FFFF_FFFE - o.0[4],
        ])
    }
...
fn bytes_sub_p(a: &mut [u8; 32]) {
    ...
    for i in 0..32 {
        let d = (a[i] as u16).wrapping_sub(P_BYTES[i] as u16).wrapping_sub(borrow);
        a[i] = d as u8;
...
fn labeled_expand(
    ...
    let mut labeled = Vec::with_capacity(
        2 + HPKE_V1.len() + suite_id.len() + label.len() + info.len(),
    );
...
pub fn poly1305_mac(msg: &[u8], key: &[u8; 32]) -> [u8; 16] {
    ...
    while pos < msg.len() {
        ...
        let w2 = u128::from_le_bytes(wide[16..24].try_into().unwrap());
        let n0 = w0 & MASK44;
...
pub fn chacha20_poly1305_open(
    ...
    if ct_tag.len() < 16 {
        return Err(DpiGuardError::OutOfRange("AEAD ciphertext too short".into()));
    }
