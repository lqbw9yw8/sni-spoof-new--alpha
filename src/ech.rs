...
pub fn seal_real_ech_hello(
    ...
    if !cfg
        ...
        .iter()
        .any(|(kdf, aead)| {
            *kdf == crate::hpke::KDF_ID_SHA256
                && *aead == crate::hpke::AEAD_ID_CHACHA20_POLY1305
        })
    {
        return Err(DpiGuardError::OutOfRange(
    ...
    ext_body.extend(std::iter::repeat(0u8).take(ct_len));
...
mod tests {
    ...
    fn real_ech_seal_hides_sni_and_is_deterministic() {
        ...
        let ech = exts.iter().find(|x| x.ext_type == ECH_EXTENSION_TYPE).unwrap();
        ...
        let enc_len = u16::from_be_bytes([
            sealed[ech.body_start + 5],
            sealed[ech.body_start + 6],
        ]) as usize;
        ...
        let other = seal_real_ech_hello(&record, b"secret.example.com", &cfg, Some([7u8; 32]))
            .unwrap();
    ...
    fn real_ech_sealed_payload_decrypts_to_inner_hello() {
        ...
        let seed = [0x42u8; 32];
        let sealed =
            seal_real_ech_hello(&record, b"secret.example.com", &cfg, Some(seed)).unwrap();
        ...
        assert!(inner.windows(5).any(|w| w == [0xFE, 0x0D, 0x00, 0x01, 0x01]));
